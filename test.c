...
int main(int argc, char **argv)
{
    ...
    int pid = 0; 			/* Holds ProcessID */
    /* Initialize Semaphore */
    LogFileSem = mmap(NULL, sizeof(sem_t), PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
    sem_init(LogFileSem, 1, 1 );	
	...
	connfd = Accept(listenfd, (SA *)&clientaddr, &clientlen);
    	/* Initialize process to handle request */
	if ( (pid = fork()) == 0 ){
		close(listenfd);	// child process closes its listening port
		HandlerFunction( connfd ); // child handles request
	}
	close(connfd); // Parent closes connected socket to listen for new request
    }
    return 0;
}
...
*/
    ...
    sem_wait(LogFileSem); // Log File Synchronization
